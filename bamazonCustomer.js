...
connection.query('SELECT product_name, department_name, price, stock_quantity FROM products', function(err, result){
	...
	for(var i = 0; i < result.length; i++){
		table.push(
			[result[i].product_name, result[i].department_name, result[i].price, result[i].stock_quantity]
		);
	}
...
var purchase = function(){
	...
	var productInfo = {
		properties: {
			product_name:{description: colors.blue('Please enter the product name of the item you wish to purchase!')},
	...
	prompt.get(productInfo, function(err, res){
		...
		connection.query('SELECT * FROM products WHERE product_name=?', productPurchased[0].product_name, function(err, res){
				...
				//otherwise if the stock amount available is more than or equal to the amount being asked for then the purchase is continued and the user is alerted of what items are being purchased, how much one item is and what the total amount is
					...
					// connection.query("UPDATE Departments SET TotalSales = ? WHERE DepartmentName = ?;", [saleTotal, res[0].department_name], function(err, resultOne){
					...
					connection.query("UPDATE products SET stock_quantity = " + newQuantity +" WHERE product_name = " + productPurchased[0].product_name, function(err, res){
						// if(err) throw err;
